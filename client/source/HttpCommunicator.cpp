...
int HttpCommunicator::loadAnnNeuronsAmount(int annId) {
	...
	strcpy(buffer, "");
	sprintf(buffer, "POST %s HTTP/1.0\nHost: %s\r\nContent-Length: %d\r\nConnection: Keep-Alive\r\nContent-Type: application/x-www-form-urlencoded\r\n\r\n%s\r\n", LOAD_NEURONS_AMOUNT_SCRIPT, HOST, strlen(parameters), parameters);
	if (send(client, buffer, strlen(buffer), 0) == SOCKET_ERROR) {
		closesocket( client );
		WSACleanup();
		throw( "HttpCommunicator00053" );
		return( 0 );
	}
